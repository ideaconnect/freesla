...
import {
  SERVICE_UUID, WRITE_UUID, NOTIFY_UUID, ROLE, KEY_FORM_FACTOR, CLOSURE_STATE
} from '../tesla/constants.js'
...
// How long after the link comes up before asking the car what it is. Long
// enough that the Bluetooth stack has finished unwinding the connection, short
// enough that the answer is on screen before anybody has looked away.
const STATUS_READ_DELAY_MS = 250

// A panel takes several seconds to travel, and the car answers the read that
// follows a command with "opening" or "closing" because at that instant it is.
// Nothing asked again, so the screen kept saying "opening" about a boot that
// had finished opening some time ago.
//
// So a reading that reports movement is followed up until the movement stops.
// The limit is what keeps it from becoming a poll: a panel that never arrives
// -- an obstruction, a boot nobody can shut remotely -- stops being asked
// about, and the last thing it said stands.
const MOTION_POLL_MS = 1400
const MOTION_POLL_LIMIT = 6
...
export function createController (options) {
  ...
  let onStatusChange = options.onStatusChange || function () {}
  ...
  let deriveSharedSecret = options.deriveSharedSecret
  ...
  // Pending "what are you" read; see scheduleStatusRead.
  let statusReadTimer = null
  // How many follow-up reads a single piece of movement has already cost.
  let motionPolls = 0
  ...
  function runCommand (name, invoke) {
    ...
    runningCommand = name
    // A fresh action gets a fresh budget of follow-up reads, whatever the last
    // one spent.
    motionPolls = 0
    // Said as an action in progress, not as a bare noun. The main screen puts
    // this line directly above a button reading Unlock, and a status that also
    // said just "Unlock" read as a label for the button rather than as a report
    // that the command was on its way.
    setState(STATE.BUSY, 'Sending ' + name + '…')
  ...
  // Asks the car what it is, once, as soon as there is a link to ask over.
  //
  // Without this the screen could show what the car is doing only after being
  // told to do something: the status line stayed blank from the moment it
  // connected until the first button was pressed, which is precisely backwards.
  // Whether the doors are locked is the question you have walking towards a
  // car, and answering it should not cost a command.
  //
  // Deferred off the Bluetooth callback rather than run inside it. Everything
  // in that callback already happens while the stack is still unwinding the
  // connection, and it is the heaviest moment in this app; building and signing
  // a command on top of it is how a watchdog reset gets earned.
  function scheduleStatusRead (delay) {
    if (statusReadTimer) return
    statusReadTimer = setTimeout(() => {
      statusReadTimer = null
      // Auto-unlock may have taken the link in the meantime, and the command it
      // sent reads the status itself when it lands.
      if (state !== STATE.READY) return
      refreshStatus()
    }, delay === undefined ? STATUS_READ_DELAY_MS : delay)
  }

  function anythingMoving (status) {
    if (!status || !status.closures) return false
    for (const field of Object.keys(status.closures)) {
      const value = status.closures[field]
      if (value === CLOSURE_STATE.OPENING || value === CLOSURE_STATE.CLOSING) return true
    }
    return false
  }

  // Keeps asking while a panel is still travelling, and stops as soon as it is
  // not. Called on every reading, including the ones these follow-ups produce,
  // so the chain ends itself the moment the car reports something settled.
  function followMotion (status) {
    if (!anythingMoving(status)) {
      motionPolls = 0
      return
    }
    if (motionPolls >= MOTION_POLL_LIMIT) {
      log('a panel is still moving after ' + motionPolls + ' reads; leaving it')
      return
    }
    motionPolls++
    scheduleStatusRead(MOTION_POLL_MS)
  }

  function cancelStatusRead () {
    motionPolls = 0
    if (!statusReadTimer) return
    clearTimeout(statusReadTimer)
    statusReadTimer = null
  }
  ...
  return {
    ...
    // Whether there is a live link to the car right now.
    //
    // Distinct from `state`, which is what this app is doing: a screen that has
    // just been opened over a working connection reads DISCONNECTED nowhere,
    // but it does need to know whether to start one.
    isLinked () { return !!transport && transport.state === BLE_STATE.READY },

    begin () {
      refresh()
    },

    // Points the controller at whichever screen is in front.
    //
    // This outlives any one of them. The link belongs to the app, so the
    // screens take turns being the thing it reports to, and a screen being torn
    // down says so rather than taking the connection with it.
    //
    // The deriver comes along for the ride because it is the one part that
    // genuinely is page-scoped: it reaches the phone through ZML's per-page
    // request machinery, which dies with the page that owns it.
    attach (handlers) {
      onChange = handlers.onChange || function () {}
      onStatusChange = handlers.onStatusChange || function () {}
      if (handlers.deriveSharedSecret) deriveSharedSecret = handlers.deriveSharedSecret
    },

    // Stops reporting to a screen that is going away. Deliberately does not
    // touch the link, the transport or the client: the next screen is about to
    // pick all three up exactly where this one left them.
    detach () {
      onChange = function () {}
