...
//
// "Once" is doing a lot of work in that sentence, so it is worth being exact
// about where that once happens. The ECDH is a P-256 scalar multiplication:
// ~14ms in this engine, ~87 seconds on the watch's interpreter. It is not run
// on the watch. The watch asks the phone, which already generates the keypair
// for the same reason, and caches what comes back forever. See createClient's
// deriveSharedSecret.
...
// The cheap half of the derivation: the curve point in, the AES-128 key out.
// This part is symmetric hashing and runs anywhere, watch included.
export function sessionKeyFromShared (sharedX) {
  if (!sharedX || sharedX.length !== 32) return null
  return sha1(sharedX).subarray(0, 16)
}

// The expensive half, run here and now.
//
// Suitable as createClient's `deriveSharedSecret` anywhere the curve maths is
// affordable -- the phone, the tests, the mock vehicle. Named for what it is so
// that wiring it into watch code reads as the mistake it would be: the reply
// that triggers a derivation arrives on a native Bluetooth callback, and
// blocking one of those for 87 seconds does not freeze the screen, it resets
// the watch.
export function localSharedSecret (privateKey, vehiclePublicKey, callback) {
  const sharedX = ecdh(privateKey, vehiclePublicKey)
  if (!sharedX) {
    callback(new Error('vehicle public key is not a valid curve point'))
    return
  }
  callback(null, sharedX)
}

// deriveSessionKey used to live here, doing both halves in one synchronous
// call. It is gone rather than merely unused: it was the shape that made the
// watch reset -- 87 seconds of curve arithmetic behind an innocuous-looking
// call, reachable from a Bluetooth callback -- and leaving it exported would
// leave that shape available to whoever needs a session key next.
