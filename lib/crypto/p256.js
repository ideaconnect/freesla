...
//
// Cost is roughly 3-4k field multiplications per scalar multiplication, which
// is seconds on watch hardware. That is affordable only because every scalar
// multiplication here is one-time and cached: key generation happens once ever,
// and the ECDH shared secret depends only on our key and the vehicle's, so it
// survives across sessions. Per-command work is symmetric only.
...
// independent of the scalar's bits; the operation is one-time and cached, so
// the price is not felt at unlock time.
